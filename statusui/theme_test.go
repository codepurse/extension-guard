...
// The window is one fixed size now, so there is no ordering left to check - only
// that it fits the screens this runs on. 1366x768 is still a common Windows
// laptop, and after the taskbar that leaves roughly 730 usable pixels of height.
// A window taller than that opens with its lower half off the screen and, with
// resizing off, no way to reach it.
func TestWindowFitsASmallLaptop(t *testing.T) {
	const usableHeightOn768 = 730
	if windowHeight > usableHeightOn768 {
		t.Errorf("windowHeight %d does not fit a 1366x768 screen once the taskbar is taken off (%d)",
			windowHeight, usableHeightOn768)
	}
	if windowWidth > 1366 {
		t.Errorf("windowWidth %d is wider than a 1366px screen", windowWidth)
	}
}
