...
// KindNames is BrowserNames for a plain Kind list: the display names, joined the
// way a sentence needs them. Extension changes name Chromium browsers as well as
// the Firefox family, and those do not arrive as GeckoBrowser values.
func KindNames(list []Kind) string {
	names := make([]string, 0, len(list))
	for _, k := range list {
		names = append(names, KindLabel(k))
	}
	switch len(names) {
	case 0:
		return ""
...
// KindLabel is a browser kind as somebody would write it. A discovered Firefox
// fork keeps the name its registration gave it; everything else is the kind with
// a capital, which is what all five of them happen to be.
