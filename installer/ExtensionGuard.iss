; Inno Setup script for Extension Guard.
;
...
;
; Build with: ISCC.exe ExtensionGuard.iss   (see README.md in this folder)
...
; repo-root VERSION file). The fallback keeps a bare `ISCC ExtensionGuard.iss`
; working.
#ifndef AppVersion
...
; The AppId below is unchanged and UsePreviousAppDir defaults to yes, so an
; existing install keeps the directory it already has and only new ones land
; here. That divergence is deliberate: moving an existing install would mean
; stopping a hardened, auto-restarting service, relocating the binary the SCM
; has a recorded path to, and re-registering it - a lot of moving parts for a
; folder name nobody reads. The fleet converges as machines are reinstalled.
...
OutputDir=output
OutputBaseFilename=ExtensionGuard-Setup
Compression=lzma2
...
; The name has moved twice: "Extension Guard" to "Ward", and now back, because
; Ward turned out to be a separate product rather than a new name for this one.
; Inno creates the current shortcuts but leaves earlier ones alone, so an
; upgraded machine would show two names pointing at the same exe. Remove the
; Ward pair. Harmless on a clean install - nothing is there to delete.
[InstallDelete]
Type: files; Name: "{autoprograms}\Ward.lnk"
Type: files; Name: "{autodesktop}\Ward.lnk"
